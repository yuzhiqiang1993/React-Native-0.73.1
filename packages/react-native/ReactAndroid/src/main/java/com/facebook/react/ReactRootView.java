...
public class ReactRootView extends FrameLayout implements RootView, ReactRoot {
    ...
    private int mRootViewTag = 0; /* This should be View.NO_ID, but for legacy reasons we haven't migrated yet */
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        try {
            boolean measureSpecsUpdated = widthMeasureSpec != mWidthMeasureSpec || heightMeasureSpec != mHeightMeasureSpec;
    ...
    public void startReactApplication(ReactInstanceManager reactInstanceManager, String moduleName, @Nullable Bundle initialProperties, @Nullable String initialUITemplate) {
        ...
        try {
            ...
            /*
             * 创建React上下文，内部关键步骤例如通过网络下载jsBundle，加载jsBundle，检查开发服务器是否在运行等是异步操作
             * 不会阻塞主线程
             *
             * */
    ...
    private void setSurfaceConstraintsToScreenSize() {
        ...
        mWidthMeasureSpec = MeasureSpec.makeMeasureSpec(displayMetrics.widthPixels, MeasureSpec.AT_MOST);
        mHeightMeasureSpec = MeasureSpec.makeMeasureSpec(displayMetrics.heightPixels, MeasureSpec.AT_MOST);
    }
    ...
    private void updateRootLayoutSpecs(boolean measureSpecsChanged, final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        if (reactApplicationContext != null) {
            @Nullable UIManager uiManager = UIManagerHelper.getUIManager(reactApplicationContext, getUIManagerType());
            ...
            if (uiManager != null) {
                ...
                if (measureSpecsChanged || offsetX != mLastOffsetX || offsetY != mLastOffsetY) {
                    uiManager.updateRootLayoutSpecs(getRootViewTag(), widthMeasureSpec, heightMeasureSpec, offsetX, offsetY);
                }
    ...
    private class CustomGlobalLayoutListener implements ViewTreeObserver.OnGlobalLayoutListener {
        ...
        private void checkForKeyboardEvents() {
            ...
            if (keyboardIsVisible != mKeyboardIsVisible) {
                ...
                if (keyboardIsVisible) {
                    ...
                    int screenY = softInputMode == WindowManager.LayoutParams.SOFT_INPUT_ADJUST_NOTHING ? mVisibleViewArea.bottom - height : mVisibleViewArea.bottom;

                    sendEvent("keyboardDidShow", createKeyboardEventPayload(PixelUtil.toDIPFromPixel(screenY), PixelUtil.toDIPFromPixel(mVisibleViewArea.left), PixelUtil.toDIPFromPixel(mVisibleViewArea.width()), PixelUtil.toDIPFromPixel(height)));
                } else {
                    sendEvent("keyboardDidHide", createKeyboardEventPayload(PixelUtil.toDIPFromPixel(mLastHeight), 0, PixelUtil.toDIPFromPixel(mVisibleViewArea.width()), 0));
                }
            }
        }
        ...
        private void checkForKeyboardEventsLegacy() {
            ...
            final int heightDiff = DisplayMetricsHolder.getWindowDisplayMetrics().heightPixels - mVisibleViewArea.bottom + notchHeight;

            boolean isKeyboardShowingOrKeyboardHeightChanged = mKeyboardHeight != heightDiff && heightDiff > mMinKeyboardHeightDetected;
